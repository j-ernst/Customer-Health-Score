...
const COLUMN_GROUPS = {
  ...
  'Activity Ratios': ['current_vs_baseline_ratio', 'current_vs_max_ratio', 'activity_consistency'],
  'Recent Activity': ['activities_last_30d', 'activities_last_7d', 'activities_last_3d', 'days_since_last_activity'],
  'Behavior Flags': ['strong_decline', 'moderate_decline', 'stable_trend', 'growing_trend', 'no_recent_activity', 'sporadic_user', 'inactive_user', 'dormant_user']
}
...
const COLUMN_CONFIG: Record<string, any> = {
  ...
  contract_expiring_soon: { label: "Vertrag läuft bald ab", type: "boolean", format: (val: any) => val ? "⚠️ Ja" : "Nein" },
  baseline_type: { label: "Baseline Typ", type: "text" },
  baseline_weekly_avg: { label: "Baseline Wöchentlich", type: "number", format: (val: number) => val?.toFixed(1) || "0.0" },
  max_weekly_activity: { label: "Max. Wochenaktivität", type: "number", format: (val: number) => val?.toLocaleString() || "0" },
  days_since_last_activity: { label: "Tage seit letzter Aktivität", type: "days_since", format: (val: number) => val?.toLocaleString() || "0" },
  activities_last_30d: { label: "Aktivitäten 30T", type: "number", format: (val: number) => val?.toLocaleString() || "0" },
  activities_last_7d: { label: "Aktivitäten 7T", type: "number", format: (val: number) => val?.toLocaleString() || "0" },
  activities_last_3d: { label: "Aktivitäten 3T", type: "number", format: (val: number) => val?.toLocaleString() || "0" }
}
...
export default function CustomerHealthDashboard() {
  ...
  // Simple CSV parser function (optimized)
  const parseCSV = useCallback((csvText: string) => {
    ...
    // Parse data rows (limit processing for performance)
    const data = lines.slice(1, Math.min(lines.length, 5001)).map(line => { // Limit to 5000 rows max
  ...
  const renderCellContent = (value: any, columnName: string) => {
    ...
    switch (config?.type) {
      case 'health_score':
        ...
        return (
          <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${getHealthScoreColor(score)}`}>
            {formattedValue}
          </div>
        )
      ...
      case 'risk_category':
        return (
          <Badge variant="outline" className={`text-xs ${getRiskCategoryColor(value)}`}>
            {formattedValue}
          </Badge>
        )
            ...
            <div className="w-12 bg-gray-200 rounded-full h-1.5">
              <div
                className={`h-1.5 rounded-full ${
                  percentage > 70 ? 'bg-red-500' : percentage > 40 ? 'bg-orange-500' : 'bg-green-500'
                }`}
                style={{ width: `${Math.min(percentage, 100)}%` }}
              ></div>
            </div>
            <span className="text-xs text-neutral-600">{formattedValue}</span>
          </div>
        )
      ...
      case 'days_since':
        ...
        return (
          <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
            days > 14 ? 'bg-red-100 text-red-800' : days > 7 ? 'bg-orange-100 text-orange-800' : 'bg-green-100 text-green-800'
          }`}>
            {formattedValue}
          </span>
        )
  ...
  const summaryStats = useMemo(() => {
    ...
    const avgHealthScore = customerData.length > 0 
      ? customerData.reduce((sum, customer) => sum + (Number(customer.health_score) || 0), 0) / customerData.length
      : 0
    ...
    return (
      <div className="min-h-screen bg-neutral-50 p-6 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-neutral-600">Lade Customer Health Daten...</p>
        </div>
      </div>
    )
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-neutral-500 mt-1">Health Score &lt; 40</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-orange-600">{summaryStats.atRisk}</div>
              <p className="text-xs text-neutral-500 mt-1">Health Score 40-69</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-neutral-500 mt-1">Health Score ≥ 70</p>
            </CardContent>
          </Card>

          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-blue-600">Ø Health Score</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-blue-600">{summaryStats.avgHealthScore.toFixed(1)}</div>
              <p className="text-xs text-neutral-500 mt-1">Durchschnitt</p>
            </CardContent>
          </Card>
        </div>
          ...
          <CardContent>
              ...
              <Table>
                ...
                <TableBody>
                  {paginatedData.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={visibleColumns.length} className="text-center py-8 text-neutral-500">
                        Keine Daten gefunden. Überprüfen Sie Ihre Suchkriterien.
                      </TableCell>
                    </TableRow>
                  ) : (
                    paginatedData.map((customer, index) => (
                      <TableRow key={customer.company_id || index} className="hover:bg-neutral-50">
                        {visibleColumns.map((column) => (
                          <TableCell key={column} className="font-medium">
                            {renderCellContent(customer[column], column)}
                          </TableCell>
                        ))}
                      </TableRow>
                    ))
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between mt-4">
                <div className="text-sm text-neutral-600">
                  Zeige {((currentPage - 1) * ITEMS_PER_PAGE) + 1} bis {Math.min(currentPage * ITEMS_PER_PAGE, totalItems)} von {totalItems} Kunden
                </div>
            ...
            <div className="mt-4 p-3 bg-neutral-50 rounded-lg">
              <div className="text-xs text-neutral-500 space-y-1">
                ...
                <p>
                  <strong>Letzte Aktualisierung:</strong> {new Date().toLocaleString('de-DE')}
                </p>
              </div>
            </div>
          </CardContent>
